...
from ._upload import DEFAULT_UPLOAD_BASE_URL, TempUploader
...
# DEFAULT_UPLOAD_BASE_URL re-exported for backwards compatibility; the temp-host
# upload logic now lives in the shared _upload.TempUploader.
...
class AikitProvider(GroqProvider):
    ...
    async def generate_with_images(
        ...
        async with httpx.AsyncClient(timeout=httpx.Timeout(60.0)) as up_client:
            try:
                for path in valid:
                    uploads.append(await self._uploader.upload(up_client, path))
                image_parts = [
                ...
                for u in uploads:
                    await self._uploader.delete(up_client, u)
