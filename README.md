...
- **Added multiple alternative LLM providers** alongside the default Gemini backend, selectable via a new `LLM_PROVIDER` setting on `AgentConfig`. The active provider only needs its own API key.
  ...
  - **Multi-key rotation** — Groq, OpenAI-compatible, and aikit all accept several comma-separated keys/tokens, rotated round-robin with 429 failover.
  - **Robustness for weaker models:** the OpenAI-compatible path samples at a low temperature (0.1) so weaker vision models emit stable, repeatable coordinates instead of jittery guesses; auto-repair of malformed grid coordinates (e.g. `["00"]` instead of `[0,0]`); and tightened prompts (stricter coordinate format + step-by-step).
  - **Bounded retries:** the failed-challenge retry loop is capped by `MAX_RETRY_ON_FAILURE` (default 5) instead of recursing without limit, so a hard site returns a result before an upstream proxy's edge timeout (Cloudflare 524) and without unbounded stack/memory growth.
